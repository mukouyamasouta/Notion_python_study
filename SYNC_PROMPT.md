# Notion 同期ルーティン（毎週金曜 19:00 JST + オンデマンド）

このリポジトリの `index.html` / `data.js` は、Notion の
**「🐍 Python知識ベース」データベース**（`https://app.notion.com/p/36fe3be8d5ac80dda919cd12d3f97555`）
の内容から生成された演習問題アプリです。このファイルは、Notionとの差分を検知して
`data.js` を更新するための手順書です。人間が読んでも実行できるし、
Claude（cloud agent、または対話セッション中のClaude Code）が読んでそのまま実行することも想定しています。

## 実行タイミング

- **毎週金曜 19:00 (Asia/Tokyo)** — `schedule` の cron ルーティンとして登録済み（自動実行）。
- **オンデマンド** — ユーザーが Claude とのチャットで「更新して」「Notionの差分を反映して」
  などと伝えたとき、その場でこの手順を実行する。アプリ内の「同期」ボタン（⟳）自体は
  静的サイトのため実際のNotionアクセスはできず、単に最新の `data.js` を再読み込みするだけ
  なので、実際の差分検知・新規問題の生成は必ずこの手順に沿ってClaudeが行う。
...
3. **変更があったページごとに問題を作り直す**
   ...
   **チャプター（用語解説）も同時にメンテナンスすること：**
   - 各トピックフォルダの先頭には `chapters[chapterId]` を指す「チャプター」リーフが1つあり、
     `{title, path, notionUrl, intro, terms:[{term, desc}, ...]}` の形。
   - 新しく作った／更新した問題に登場する語句・関数・概念のうち、
     **`data.js` の `chapters` 全体を通してまだ一度も `term` として登場していないもの**だけを、
     そのページのチャプターの `terms` に追記する（＝差分のみ。重複させない）。
   - 判定は用語の表記（例: `sorted()`, `self`, `辞書（dict）`）単位で行う。
     同じ概念でも既存のチャプターで説明済みなら書かない。判断に迷ったら、
     全チャプターの `terms[].term` を一覧して重複していないか確認してから追記する。
   - 新規トピックページ（これまで存在しなかったページ）を追加する場合は、
     新しいチャプターID（例: `ch-xxx`）を作り、`tree` の該当フォルダの先頭に
     `{"id": "ch-xxx", "type": "chapter"}` を追加する。
